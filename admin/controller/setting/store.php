...
class ControllerSettingStore extends Controller {
	...
	protected function getForm() {
		...
		} elseif (isset($store_info['config_footerlogo'])) {
			$data['config_footerlogo'] = $store_info['config_footerlogo'];
		} else {
			$data['config_footerlogo'] = '';
		}

		if (isset($this->request->post['config_footerlogo']) && is_file(DIR_IMAGE . $this->request->post['config_footerlogo'])) {
			$data['footerlogo'] = $this->model_tool_image->resize($this->request->post['config_footerlogo'], 100, 100);
		} elseif (isset($store_info['config_footerlogo']) && is_file(DIR_IMAGE . $store_info['config_footerlogo'])) {
			$data['footerlogo'] = $this->model_tool_image->resize($store_info['config_footerlogo'], 100, 100);
		} else {
			$data['footerlogo'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
