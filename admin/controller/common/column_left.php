...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ($this->request->get['user_token'] == $this->session->data['user_token'])) {
			...
			if($this->config->get('module_gallery_album_status')){
                if ($this->user->hasPermission('access', 'extension/module/gallery_album')) {   
					$catalog[] = array(
					  'name'     => $this->language->get('text_gallery_album'),
					  'href'     => $this->url->link('catalog/gallery_album', 'user_token=' . $this->session->data['user_token'], true),
					  'children' => array()   
					);          
				}
			}
