<?php
class ModelThemetechmountArticle extends Model {
	public function createBlogs() {
		$create_blogger = "CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "blogger` (`blogger_id` int(11) NOT NULL auto_increment, `module_id` int(11) NOT NULL, `status` int(1) NOT NULL default '0', `author` varchar(255) default NULL, `image` varchar(255) default NULL, `date_added` datetime NOT NULL default '0000-00-00 00:00:00', `date_modified` datetime NOT NULL default '0000-00-00 00:00:00', PRIMARY KEY (`blogger_id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;";
	...
	public function addBlog($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "blogger SET module_id = '" . (int)$data['module_id'] . "', status = '" . (int)$data['status'] . "', author = '" . $this->db->escape($data['author']) . "', date_added = now(), date_modified = now()");
	...
	public function editBlog($blogger_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "blogger SET module_id = '" . (int)$data['module_id'] . "', status = '" . (int)$data['status'] . "', author = '" . $this->db->escape($data['author']) . "', date_modified = now() WHERE blogger_id = '" . (int)$blogger_id . "'");
