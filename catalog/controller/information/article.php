...
class ControllerInformationArticle extends Controller
{
    ...
    public function index()
    {
        ...
        if (!isset($this->request->get['blogger_id'])) {
            $this->response->redirect($this->url->link('information/article/articlelist'));
        }
        $language = $this->model_themetechmount_article->getLanguageByCode($this->session->data['language']);
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $this->model_themetechmount_article->addComment($this->request->get['blogger_id'], $this->request->post, $language['language_id']);
        ...
        if ($blog_info) {
            ...
            $data['breadcrumbs'][]       = array(
                ...
                'href' => $this->url->link('information/article', '', true)
            );
            $data['breadcrumbs'][]       = array(
                'text' => $blog_info['title'],
                'href' => $this->url->link('information/article', 'blogger_id=' . $this->request->get['blogger_id'], true)
            );
            $data['heading_title']       = $blog_info['title'];
            $data['date']                = date('j F, Y', strtotime($blog_info['date_added']));
            $module                      = $this->model_themetechmount_article->getModule($blog_info['module_id']);
            ...
            $blog_comments         = $this->model_themetechmount_article->getBlogComments($this->request->get['blogger_id'], $language['language_id']);
            if ($blog_comments) {
            ...
            $data['action'] = $this->url->link('information/article', 'blogger_id=' . $this->request->get['blogger_id'], true);
            ...
            if (!$this->customer->isLogged()) {
                $this->session->data['redirect'] = $this->url->link('information/article', 'blogger_id=' . $this->request->get['blogger_id'], true);
            }
    ...
    public function articlelist()
    {
        ...
        $data['articles']           = array();
        $articles                   = $this->model_themetechmount_article->getBlogs();
        foreach ($articles as $blog) {
            $module_blog_data = array();
            $module           = $this->model_themetechmount_article->getModule($blog['module_id']);
            $setting          = json_decode($module['setting'], true);
            if (!empty($setting['status']) && $setting['status']) {
                $module_blogs = $this->model_themetechmount_article->getBlogsByModule($blog['module_id']);
                foreach ($module_blogs as $module_blog) {
                    $total_comments     = $this->model_themetechmount_article->getTotalBlogComments($module_blog['blogger_id']);
                    $module_blog_data[] = array(
                        'title' => $module_blog['title'],
                        'image' => $this->model_tool_image->resize($module_blog['image'], $setting['width'], $setting['height']),
						'author' => html_entity_decode($module_blog['author']),
                        ...
                        'href' => $this->url->link('information/article', 'blogger_id=' . $module_blog['blogger_id'], true)
                    );
                }
                $data['articles'][] = array(
                    'name' => $module['name'],
        ...
        $data['footer']         = $this->load->controller('common/footer');
        $data['header']         = $this->load->controller('common/header');
        $this->response->setOutput($this->load->view('information/article_list', $data));
    }
}