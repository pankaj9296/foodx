...
class ModelThemetechmountArticle extends Model
{
    ...
    public function getBlogs()
    {
        ...
        if ($query->rows) {
            foreach ($query->rows as $blog) {
                if ($this->getLayoutModule('article.' . $blog['module_id'])) {
                    $blog_data[] = $blog;
                }
            }
        }
    ...
    /* public function getTotalBlogAuthor($blogger_id) {
    $query = $this->db->query("SELECT `author` AS total FROM " . DB_PREFIX . "blogger_comment WHERE blogger_id = '" . (int)$blogger_id . "'");
